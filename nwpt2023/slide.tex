...
\begin{frame} [fragile]
  ...
  A relation $R$ is \emph{butterfly} if and only if $R$ satisfies the condition represented by the following diagram:
  \begin{center}
  \begin{tikzcd}
    d_{1} & e_{1} \\
    d     & e     \\
...
\begin{frame} [fragile]
  ...
  The \emph{butterfly} property of relations is closed under $\cup$ but \red{not} closed under $\bowtie$.
...
\begin{frame} [fragile]
  \frametitle{Solution 2 -- Norrowing the class of relations }
...
\begin{frame} [fragile]
  ...
  \end{tikzcd}
  \end{center}
  And indeed the class of relations is closed under both $\cup$ and $\bowtie$.
\end{frame}
