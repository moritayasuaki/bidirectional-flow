...
instruction or a bigger program is a relation. If the data-flow
domains for the entry and exit points are $(D, \leq, \bigjoin)$,
$(E, \leq, \bigjoin)$, the corresponding compatibility relation is a
subset of their product, i.e., an element of the set
$\Pow(D \times E)$, which is also a complete lattice; we typically
denote relations by $R$, $S$.  And then we aim to specify such a
compatibility relation by a pair of monotone backward and forward
...
This approach does not always allow one to capture the original bidirectional analysis exactly. For example, in the load-pop reduction analysis, the compatibility relations for all single instructions are butterfly, but those for some sequences (e.g., \textsf{load}; \textsf{dup}) are not. Still, this approach can provide a reasonable systematic overapproximation of the original analysis in some cases. 
