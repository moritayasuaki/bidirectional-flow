...
% 
  ...
  where
  \begin{align*}
    P'(\fb , \ff) &= (\fb , \ff(\top)) & M'(\fb , e) &= (\fb , \lambda d. e) \\
...
$(\fb, c)$ is in the image of $P' \comp J \comp H$ if and only if $(\fb, c) \leq P'(J(H(I(K(M'(\fb , c))))))$, meaning
\begin{align*}
  \fb(e) &\leq \pi_{0} (\bigmeet \{(d' , e') \mid (d' , e') \leq (\fb(e'), e' \meet c) \}) \\
  c &\leq \pi_{1} (\bigmeet \{(d' , e') \mid (d' , e') \leq (\fb(e'), c) \})
\end{align*}
for any $e$. But the second condition is always true because $\pi_{1} (\bigmeet \{(d' , e') \mid (d' , e') \leq (\fb(e'), c) \}) = c$.
Define $\chi(d', e') = (\fb(e') , e' \meet c)$.
Then the condition becomes
\begin{align*}
  \fb(e) &\leq \pi_{0} (\nu \chi) \\
\end{align*}
Let us write $(d_{*} , e_{*}) = \nu \chi$, then we get
$\fb(e) \leq d_{*} = \fb(e_{*}) = \fb(e \meet c) \leq \fb(c)$.
Thus, we proved that if $(\fb, c)$ is in the image of $P' \comp J \comp H$ then $\fb(e) \leq \fb(c)$ for any e.
Conversely, if $\fb(e) \leq \fb(c)$, we get $\fb(e) \leq \fb(c) \leq \pi_{0} (\nu \chi)$ from the fact $(\fb(c) , c)$ is a fixed point of $\chi$.
