...
f
 ...
 Lifting of composition in poset of backward transfer functions
    ...
    We can see that taking pointwise meet of backward functions gives an ideal approximation for non-deterministic choice within the image of $G$:
    \begin{proposition}\label{prop:backword-closed-under-intersection}
      ...
      \begin{proof}
        ...
        We see that the intersection $G(\fb) \cap G(\gb)$ is equals to $G(\fb \meet \gb)$. Therefore, the image of $G$ is closed under $\cap$.
      \end{proof}
    \end{proposition}
    Notice the above proof only relies on general properties of Galois connections.
    Thus, the image of any upper adjoint with domain $\Rel(D, E)$ and codomain $Y$ is closed under $\cap$ if $Y$ has $\meet$.
