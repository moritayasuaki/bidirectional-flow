...
f
 ...
 Lifting of composition in poset of backward transfer functions
    ...
    \begin{proposition}\label{prop:backword-closed-under-intersection}
      ...
      \begin{proof}
        Similarly to Proposition \ref{prop:backword-closed-under-composition}, the proposition is equivalent to the condition $\caplift$ underapproximates $\cap$, which means:
        \[
          d \leq \fb (e) \meet (\gb (e)) \implies d \leq \fb(e) \land d \leq \gb(e).
        \] for any $d$ and $e$.
        This follows from the essential properties of $\meet$. Thus, we conclude the image of $G$ is closed under $\cap$. \qed
      \end{proof}
    \end{proposition}
...
We show that if $(c , e) \leq (c_{*} , e_{*})$ then $(c, d) \leq f (c , d)$ and $(d, e) \leq g (d, e)$ when $d = d_{*}$.
Unfolding $\nu$ once, we obtain equations componentwisely, as follows:
\begin{align}
...
\end{align}
We get $c_{*} \leq c$ and $d_{*} \leq d$ from (\ref{eq:ineq-first}) and (\ref{eq:ineq-third}) respectively by using the property of $\meet$. Since $(c , e) \leq (c_{*} , e_{*})$ from assumption, we also get the equations $c_{*} = c$ and $e_{*} = e$.
By substituting $c$ for $c_{*}$ in the equation (\ref{eq:ineq-first}), we also get $c = c \meet \pi_{1}(f(c, d_{*})) \leq \pi_{1}(f(c, d_{*}))$. Similarly, $e \leq \pi_{2}(g(d_{*} , e))$.
By substituting $c$ and $e$ for $c_{*}$ and $e_{*}$ in (\ref{eq:ineq-second}), we have $d_{*} = \pi_{2}(f (c , d_{*})) \meet \pi_{1}(g (d_{*} , e))$. From the property of $\meet$, $d_{*} \leq \pi_{2}(f (c , d_{*})$ and $d_{*} \leq \pi_{1}(g (d_{*} , e))$. From the above discussion, we get $(c, d_{*}) \leq f(c , d_{*})$, $(d_{*}, e) \leq g(d_{*} , e)$. Therefore, the image of $I$ is closed under $\bowtie$. \qed
