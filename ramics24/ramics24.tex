...
\section{Unidirectional analyses}
  ...
  \begin{align}
    ...
    x \leq G(F(x)) \\
    F(G(y)) \leq y \\
    \exists y, x = G(y) \iff G(F(x)) = x \\
    \exists x, y = F(x) \iff F(G(y)) = y \\
...
\paragraph{The class of relations via the Galois connection}
  ...
  We say binary operation $\otimes$ on backward transfer functions is \emph{complete} with regard to $*$ on binary operation on relations if and only if
  $G(\fb) * G(\gb) \subseteq G (\fb \otimes \gb)$.

  Similarly, we say binary operation $\otimes$ on backward transfer functions is \emph{sound} with regard to $*$ on binary operation on relations if and only if
  $G (\fb \otimes \gb) \subseteq G(\fb) * G(\gb)$

  For any (partially defined) binary operation $*$ on relations can be lifted to a binary operation on backward transfer function by $F \dashv G$:
  \begin{align*}
  \fb \otimes \gb = F (G(\fb) * G(\gb))
  \end{align*}
  Lifted operation $\otimes$ is always complete:
  \begin{align*}
  G(\fb) * G(\gb) \subseteq G(F(G(\fb) * G(\gb))) = G(\fb \otimes \gb)
  \end{align*}
...
\section{Bidirectional data-flow analyses}  
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  \begin{definition}[Intermediate Adjoints]
    \begin{align*}
    H &: \Rel \to \Endo &
    I &: \Endo \to \Rel \\
